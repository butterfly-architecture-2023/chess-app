...
final class PawnTests: XCTestCase {
    ...
    func test_canMove메서드_성공케이스_블랙폰() {
        ...
        XCTAssertTrue(blackPawn.canMove(to: destination, pieces: self.pieceManager.pieces))
    }
    ...
    func test_canMove메서드_실패케이스_블랙폰_아래로_내려가지_않을_때() {
        ...
        XCTAssertFalse(blackPawn.canMove(to: destination, pieces: self.pieceManager.pieces))
    }
    ...
    func test_canMove메서드_성공케이스_화이트폰() {
        ...
        XCTAssertTrue(whitePawn.canMove(to: destination, pieces: self.pieceManager.pieces))
    }
    ...
    func test_canMove메서드_실패케이스_화이트폰_위로_올라가지_않을_때() {
        ...
        XCTAssertFalse(whitePawn.canMove(to: destination, pieces: self.pieceManager.pieces))
    }
